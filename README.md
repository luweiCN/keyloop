...
KeyLoop 是一个个人化的程序员终端打字训练工具。它不是通用打字游戏：打开后先显示练习菜单，可以选择“综合练习”按今日动态计划走完整流程，也可以只练某一个步骤。
...
有历史记录后，综合练习会根据最近 21 天的错误热区动态调整：高错键位会插入基础专项，高错符号会增加符号组，高错词/标识符会进入词块和程序员词，高慢项会回到完整代码块里练。每组完成页会显示下一组的安排原因。
...
- TS / JS / Vue / Solidity / Rust / HTML / CSS / Less / Sass 代码块，每种语言至少 120 条内置练习

代码块优先使用完整代码块，不再只抽单行碎片。默认启动会直接使用内置代码语料池，不扫描当前目录；只有传入 `--repo /path/to/project` 时，才会额外扫描指定仓库并和内置语料混合。代码缩进会在进入练习前做归一化：去掉整段共同的外层缩进，保留函数体、HTML 树、CSS 嵌套等相对缩进。代码模式下按 Enter 后会自动补齐下一行前导缩进空格，不需要像真实编辑器外那样一个个敲缩进。
