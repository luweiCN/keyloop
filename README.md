...
- TS / JS / Vue / Solidity / Rust / HTML / CSS / Less / Sass 代码块，每种语言至少 50 条内置练习

代码块优先使用完整代码块，不再只抽单行碎片。默认启动会直接使用内置代码语料池，不扫描当前目录；只有传入 `--repo /path/to/project` 时，才会额外扫描指定仓库并和内置语料混合。代码缩进会在进入练习前做归一化：去掉整段共同的外层缩进，保留函数体、HTML 树、CSS 嵌套等相对缩进。
