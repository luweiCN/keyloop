...
代码块优先使用完整语句、函数块和结构清晰的多行片段。默认启动会直接使用内置代码语料池；传入 `--repo /path/to/project` 时，会额外扫描指定仓库并和内置语料混合。代码缩进会在进入练习前做归一化：去掉整段共同的外层缩进，保留函数体、HTML 树、CSS 嵌套等相对缩进。代码模式下按 Enter 后会自动补齐下一行前导缩进空格，接近真实编辑器里的输入节奏。
