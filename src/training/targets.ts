 ...
 * 内容存放在 programming_basics/symbol_supplements.jsonl；这里仅负责加载文本。
 * 调用方负责随机打乱。
 */
export function symbolSupplementLines(
  _context: BuildTargetContext,
  options: ProgrammingBasicsOptions = {},
): string[] {
  return loadProgrammingBasicsSymbolSupplements(options).map((supplement) => supplement.text);
}
