...
describe("TS CLI parser parity", () => {
  ...
  test("vocab commands parse", () => {
    ...
    expect(parseCliArgs(["vocab", "list"]).command).toEqual({
      kind: "vocab",
      action: { kind: "list" },
    });
    expect(parseCliArgs(["vocab", "remove", "vocab-1"]).command).toEqual({
      kind: "vocab",
      action: { kind: "remove", id: "vocab-1" },
    });
    expect(parseCliArgs(["vocab", "import", "/tmp/words.json"]).command).toEqual({
      kind: "vocab",
      action: { kind: "import", path: "/tmp/words.json" },
    });
  });

  test("vocab add options accept equals values", () => {
    expect(
      parseCliArgs([
        "vocab",
        "add",
...
describe("TS CLI command dispatch", () => {
  test("help runs without creating data files", async () => {
    ...
    try {
      ...
      expect(result.stdout).toContain("keyloop vocab");
    ...
    try {
      ...
      const result = await runCli(["--language", "en"], {
        ...
        appRunner: async (context) => {
          ...
          return {
            state: { language: context.language, route: { screen: "main_menu" } },
            action: "quit",
          };
        },
      });
      ...
      expect(calls[0]?.records.map((record) => record.id)).toEqual([
        "existing-session",
      ]);
    } finally {
      await rm(dir, { recursive: true, force: true });
  ...
  test("bare keyloop persists everyday settings and starts with returned state plan", async () => {
    const dir = await tempDir();
    let runnerDailyPlan: DailyPracticePlan | undefined;
    try {
      await savePreferencesToPath(defaultPreferences("en"), preferencesPath(dir));
      ...
      await runCli(["--language", "en"], {
        env: { KEYLOOP_HOME: dir },
        now: new Date("2026-06-05T04:00:00.000Z"),
        idFactory: () => "everyday-settings",
        appRunner: async (context) => {
          const main = createOpenTuiInitialState(context.language, {
            everydaySettings: {
              word_range: "1000",
              word_count: 50,
              sentence_level: "cet4",
          ...
          const running = reduceOpenTuiAppKey(
            main,
            key("1", "1"),
            context,
          );
        ...
        runner: async (context) => {
          runnerDailyPlan = context.dailyPlan;
          return { completedRecords: [] };
        },
      });
      ...
      expect(everydayLesson?.target.source).toContain(
        "mix:words-1000:count-50:sentences-cet4:short",
      );
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  ...
  test("bare keyloop persists word form settings and starts with returned state plan", async () => {
    const dir = await tempDir();
    let runnerDailyPlan: DailyPracticePlan | undefined;
    try {
      await savePreferencesToPath(defaultPreferences("en"), preferencesPath(dir));
      ...
      const preferences = await loadPreferencesFromPath(preferencesPath(dir));
      expect(preferences.word_breakdown).toEqual({
        enabled_in_comprehensive: false,
        max_items_per_group: 8,
      });
      expect(preferences.personal_vocabulary).toEqual({
        enabled_in_comprehensive: false,
        daily_review_limit: 12,
      });
      const programmingLesson = runnerDailyPlan?.lessons.find(
        (lesson) => lesson.module === "programming_basics",
      );
      expect(programmingLesson?.target.text).not.toContain("serial ization");
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  }, 10_000);
  ...
  test("bare keyloop my vocabulary works when comprehensive vocabulary is disabled", async () => {
    const dir = await tempDir();
    let runnerDailyRunId: string | undefined;
    let runnerTargetText = "";
    try {
      await savePreferencesToPath(
        {
          ...defaultPreferences("en"),
      ...
      await runCli(["--language", "en"], {
        ...
        now: new Date("2026-06-05T04:00:00.000Z"),
        appRunner: async (context) => {
          const initialState = createOpenTuiInitialState(context.language, {
            wordFormSettings: {
              word_breakdown: context.wordBreakdownSettings ?? {
                enabled_in_comprehensive: true,
                max_items_per_group: 6,
              },
              personal_vocabulary: context.personalVocabularySettings ?? {
                enabled_in_comprehensive: true,
                daily_review_limit: 8,
              },
            },
          });
          const programmingMenu = reduceOpenTuiAppKey(
            initialState,
            key("4", "4"),
            context,
          );
          const running = reduceOpenTuiAppKey(
            programmingMenu.state,
            key("5", "5"),
            context,
          );
          if (running.action !== "start") {
            throw new Error("expected my vocabulary start action");
          }
          return { state: running.state, action: "start" };
        },
        runner: async (context) => {
          runnerDailyRunId = context.dailyPlan.run_id;
          runnerTargetText = context.dailyPlan.lessons[0]?.target.text ?? "";
          return { completedRecords: [] };
        },
      });

      expect(runnerDailyRunId).toBe("");
      expect(runnerTargetText).toContain("serial ization");
      expect(runnerTargetText).toContain("serialization serialization");
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });

  test("start builds daily plan and delegates to injected runner", async () => {
    const dir = await tempDir();
    const calls: Array<{
    ...
    try {
      ...
      expect(calls).toHaveLength(1);
      expect(calls[0]?.dailyPlan.run_id).toBe("20260605-1-cli");
      expect(calls[0]?.dailyPlan.lessons).toHaveLength(4);
      expect(calls[0]?.language).toBe("en");
      expect(calls[0]?.codeConfig).toMatchObject({
        languages: ["typescript"],
        frameworks: ["react"],
        projects: [],
        match_any: true,
      });
      expect(result.stdout).toContain("No completed sessions were saved.");
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
  ...
  test("start CLI filters override global preferences", async () => {
    const dir = await tempDir();
    const calls: CodePracticeConfig[] = [];
    try {
      await savePreferencesToPath(
        {
          ...
          speed_unit: "wpm",
          pinned_code_filters: [],
          global_code_filters: [{ facet: "framework", value: "react" }],
          code_practice: {
            difficulty: "adaptive",
            length: "adaptive",
          },
      ...
      await runCli(["start", "--code-language", "rust"], {
        env: { KEYLOOP_HOME: dir },
        now: new Date("2026-06-05T04:00:00Z"),
        runner: async (context) => {
          calls.push(context.codeConfig);
          return { completedRecords: [] };
        },
      });
  ...
  test("start injects stored personal vocabulary into programming basics", async () => {
    const dir = await tempDir();
    try {
      await saveVocabularyStoreToPath(
        {
      ...
      let programmingLessonText = "";
      await runCli(["start"], {
        env: { KEYLOOP_HOME: dir },
        now: new Date("2026-06-05T04:00:00.000Z"),
        runner: async (context) => {
          programmingLessonText =
      ...
      expect(programmingLessonText).toContain("serial ization");
      expect(programmingLessonText).toContain("serialization serialization");
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });

  test("start honors disabled word breakdown comprehensive preference", async () => {
    const dir = await tempDir();
    try {
      await savePreferencesToPath(
        {
      ...
      expect(programmingLessonText).not.toContain("config uration");
      expect(programmingLessonText).not.toContain("configuration configuration");
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });

  test("vocab add writes a normalized vocabulary entry", async () => {
    const dir = await tempDir();
    try {
      ...
      const store = await loadVocabularyStoreFromPath(vocabularyPath(dir));
      expect(result.stdout).toContain("vocab-1");
      expect(result.stdout).toContain("internationalization");
  ...
  test("vocab list prints active entries and hides archived entries", async () => {
    const dir = await tempDir();
    try {
      await saveVocabularyStoreToPath(
        {
      ...
      const result = await runCli(["--language", "en", "vocab", "list"], {
        env: { KEYLOOP_HOME: dir },
      });

      expect(result.stdout).toContain("Personal vocabulary");
      expect(result.stdout).toContain("active");
      expect(result.stdout).toContain("performance");
      expect(result.stdout).not.toContain("obsoleteTerm");
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });

  test("vocab remove archives an entry", async () => {
    const dir = await tempDir();
    try {
      await saveVocabularyStoreToPath(
        {
      ...
      const result = await runCli(["--language", "en", "vocab", "remove", "vocab-1"], {
        env: { KEYLOOP_HOME: dir },
        now: new Date("2026-06-05T04:00:00.000Z"),
      });
  ...
  test("vocab import accepts strings and partial entry objects", async () => {
    const dir = await tempDir();
    try {
      const importPath = join(dir, "words.json");
      await writeFile(
        importPath,
        JSON.stringify([
          "performance",
      ...
      const result = await runCli(["--language", "en", "vocab", "import", importPath], {
        env: { KEYLOOP_HOME: dir },
        now: new Date("2026-06-05T04:00:00.000Z"),
      ...
      const store = await loadVocabularyStoreFromPath(vocabularyPath(dir));
      expect(result.stdout).toContain("Imported 2 vocabulary entries");
      expect(store.entries.map((item) => item.text)).toEqual([
        "performance",
        "selectedReceipt",
      ]);
