...
describe("target generation core", () => {
  ...
  test("programming basics mix injects due personal vocabulary before built-in long words", () => {
    const record = defaultSessionRecord({
      token_stats: [
        {
          token: "internationalization",
    ...
    const personalIndex = target.text.indexOf("serial ization");
    const builtInIndex = target.text.indexOf("international ization");
    expect(personalIndex).toBeGreaterThanOrEqual(0);
    expect(builtInIndex).toBeGreaterThanOrEqual(0);
    expect(personalIndex).toBeLessThan(builtInIndex);
    expect(target.text).toContain("serialization serialization");
  ...
  test("programming basics mix caps personal vocabulary injection by limit", () => {
    const target = buildProgrammingBasicsMixTarget({
      records: [],
      plan: testPlan(),
      library: testLibrary(),
    ...
    expect(target.text).toContain("config uration");
  ...
  test("standalone long-word breakdown follows personal focus built-in order", () => {
    const target = buildLongWordBreakdownPracticeTarget(
      ...
      { profile: "standalone", domain: "programming", maxItems: 3 },
    );

    expect(target.mode).toBe("words");
    expect(target.source).toBe("keyloop:module:word-breakdown:serialization");
    expect(target.text).toContain("serial ization");
    expect(target.text).toContain("serialization serialization");
    expect(target.text).toContain("ser serialization");
    expect(target.text).not.toContain(" ser  serialization");
    expect(target.text).toContain("selected preference");
    expect(target.text).toContain("loadSelectedPreference");
    expect(target.text).toContain("international ization");
    expect(target.text.indexOf("serial ization")).toBeLessThan(
      target.text.indexOf("selected preference"),
    );
    expect(target.text.indexOf("selected preference")).toBeLessThan(
      target.text.indexOf("international ization"),
    );
  ...
  test("personal vocabulary target ranks active entries and expands parts", () => {
    const record = defaultSessionRecord({
      target_text: "selectedReceipt",
      token_stats: [
        {
          token: "serialization",
          kind: "word",
    ...
    const target = buildPersonalVocabularyPracticeTarget(
      [
        {
          id: "archived",
          text: "archivedTerm",
    ...
    expect(target.mode).toBe("words");
    expect(target.source).toBe("keyloop:module:personal-vocabulary");
    expect(target.text).toContain("serial ization");
    expect(target.text).toContain("serialization serialization");
    expect(target.text).toContain("selectedReceipt selectedReceipt");
    expect(target.text).not.toContain("archivedTerm");
    expect(target.text.indexOf("serialization")).toBeLessThan(
      target.text.indexOf("selectedReceipt"),
    );
  });

  test("personal vocabulary target respects max item limit", () => {
    const target = buildPersonalVocabularyPracticeTarget(
      [
        {
          id: "configuration",
  ...
  test("daily everyday mix injects due everyday vocabulary before built-in long words", () => {
    const library = testLibrary();
    library.long_words = [
      {
        word: "communication",
    ...
    expect(everyday?.target.text).toContain("collaboration collaboration");
    expect(everyday?.target.text).toContain("communi cation");
    expect(everyday?.target.text.indexOf("collabor ation")).toBeLessThan(
      everyday?.target.text.indexOf("communi cation") ?? -1,
    );
    expect(programming?.target.text).not.toContain("communi cation");
  });
  ...
  test("daily plan omits word breakdown injection when disabled in preferences", () => {
    ...
    expect(everyday?.target.text).not.toContain("collabor ation");
  ...
  test("daily plan caps word breakdown injection by preference", () => {
    ...
    expect(everyday?.target.text).toContain("collabor ation");
    expect(everyday?.target.text).not.toContain("communi cation");
    expect(programming?.target.text).toContain("config uration");
