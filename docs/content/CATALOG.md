...
TUI 里的“综合练习”不再固定理解为 1-7 组。没有历史记录时使用默认学习路径；有最近 21 天历史后，会根据 `error_chars`、`key_events`、`error_tokens`、`token_stats` 自动调整今日组别。高错键位会触发 `Foundation` 组，高错符号会增加 `Symbols` 权重，高错词/标识符会优先进入 `Chunks` 和 `Words`，慢项/错项会进入完整代码块。每个 `PracticeLesson` 都带中英文 `reason`，用于计划页和完成页说明安排原因。
