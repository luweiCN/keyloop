...
- `everyday_english.json`：KeyLoop 自有手写清洁语料，覆盖日常/职场常见词、短语和长短句，并保留 `source_id`、license 和字段范围。
...
可以用 `KEYLOOP_EVERYDAY_CORPUS=/path/to/everyday.json` 合并用户本地日常英语语料。文件结构与 `content/everyday_english.json` 相同，适合放自己的常用句、工作表达和需要反复练的词块；用户本地文件不会被写回仓库。
...
- 无 license 的项目不复制内容。
- 外部打字网站的词库、书摘或句子不做改写搬运；需要扩充时使用自有写作、公开许可材料或用户本地私有语料。
...
Everyday English lives in `content/everyday_english.json`. It is a KeyLoop hand-authored clean corpus for everyday and workplace words, phrases, and short/medium/long sentences, with source metadata and field coverage.
...
to inspect recommended source repositories and corpus license metadata. Set `KEYLOOP_EVERYDAY_CORPUS=/path/to/everyday.json` to merge a local private everyday-English corpus using the same schema as `content/everyday_english.json`.
