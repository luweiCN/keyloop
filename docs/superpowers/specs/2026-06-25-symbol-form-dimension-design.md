...
把 `symbols_numbers` 的选材从「全卡池混选 10 张」拆成 **value 路 + 非 value 路**，合并：

1. **value 路（形式覆盖）**：从 value 卡里按 format **分组 round-robin** 取 `Nv` 张（每轮各 format 取 1，组内用弱键加权 `weightedSampleWithoutReplacement` 排序）→ 保证 Nv 张尽量是**不同形式**。`Nv` 默认 **4**（对齐 `VALUES_PER_LINE=4`，正好聚成 1 行 4 个不同形式裸值；可调）。format 种类 < Nv 时取满可用种类。
2. **非 value 路（字符靶向）**：从 statement/block 卡用现有 `pickWeakKeyTargetedCards` 取 `10 - 实际value数` 张。
3. 合并 → `symbolsNumbersText`（已把 value 聚合排最前、不高亮）正常组装。

无弱键时：value 路 round-robin 仍保证形式覆盖，组内退化为随机；非 value 路退化为均衡随机。即**形式覆盖恒生效，字符靶向有弱键才生效**，两者正交叠加。

value 卡池为空（某语言无 value 卡）时：Nv=0，全走非 value 路（行为=阶段三现状），不报错、不空卷。
...
| `programmingBasicsTargets.ts` | 新增 `pickFormCoveredValueCards()`（format round-robin + 弱键加权）；`basicsTarget` 的 symbols_numbers 路径改「value 路 + 非 value 路」合并 |
